...
async function combineImages(image1: Uint8Array, image2: Uint8Array, outputPath: string): Promise<void> {
...
function normalizeFileName(lineNumber: number, url: string): string {
  const paddedNumber = lineNumber.toString().padStart(3, '0');
  const normalizedUrl = url.replace(/[^a-z0-9]+/gi, '-').toLowerCase().replace(/^-+|-+$/g, '');
  return `${paddedNumber}-${normalizedUrl}.png`;
}

async function processUrl(lineNumber: number, url: string): Promise<string> {
  const leftUrl = new URL(url, BASE_URL_LEFT).toString();
  const rightUrl = new URL(url, BASE_URL_RIGHT).toString();

  console.log(`Processing ${lineNumber}: ${url}`);
  const [screenshot1, screenshot2] = await Promise.all([
    takeScreenshot(leftUrl),
    takeScreenshot(rightUrl)
  ]);

  const outputFileName = normalizeFileName(lineNumber, url);
...
async function appendToOutputFile(url: string, outputFileName: string): Promise<void> {
...
async function main() {
  ...
  for (let i = 0; i < urls.length; i++) {
    ...
    if (!processedUrls.has(url)) {
      const outputFileName = await processUrl(i + 1, url);
